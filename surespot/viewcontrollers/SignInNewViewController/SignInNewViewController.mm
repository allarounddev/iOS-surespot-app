...
//  surespot
//
//  Created by Gevorg Karapetyan on 7/17/16.
...
@interface SignInNewViewController ()
...
- (void)viewDidLoad
{
    ...
    [_userNameTextField setPlaceholder:NSLocalizedString(@"username", nil)];
    [_textPassword setPlaceholder:NSLocalizedString(@"password", nil)];
    [_bLogin setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    ...
    if(index < [_identityNames count]) {
        userName = [_identityNames objectAtIndex:index];
    }
    
    [self updatePassword:userName];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if(_userNameTextField == textField) {
        ...
        [_textPassword becomeFirstResponder];
    } else if(_textPassword == textField) {
        [self login: nil];
        [textField resignFirstResponder];
    }
